...
function checkMail() {
    var formEmailC = /^[A-Z,0-9]/;
    var aIndex = email.value.indexOf('@');
    /* var valueEmail = email.value.split('@')[1]; */
    if (email.value.length < 8 && !formEmailC.test(email.value)) {
        document.getElementById('notification-email').innerHTML = "Tài khoản không hợp lệ";
...
passWord.onblur = function() {
   ...
   // var dbfilter = /[@\$\&\#\%]/;
    ...
    return false;
    //else if (!filter.test(passWord.value) || !dbfilter.test(passWord.value)) {
      //  document.getElementById('notification-password').innerHTML = "Mật khẩu phải có ít nhất 1 chữ hoa và 1 ký tư đặc biệt!";
   // }
}
